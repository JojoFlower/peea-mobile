...
export default function MentoratScreen() {
  ...
  const { me, upsertMentorship, refresh, loading, error } = useAuth();
  ...
  // it never fights a deliberate re-render.
  ...
  // (mentee side) or a newly-assigned mentee (mentor side) appears without
  // restarting the app. Attribution is done by the PEEA team in the admin.
  useFocusEffect(
    useCallback(() => {
  ...
  // touching the role — the assigned binôme is preserved because upsertMentorship
  // only resets when the role actually changes (which the app no longer allows).
  const goEditDetails = () => {
  ...
  // ── MATCH ────────────────────────────────────────────────────────────────
  // Attribution is handled by the PEEA team in the admin. The app only shows the
  // resulting binôme: a mentee sees their assigned mentor, a mentor sees their
  // assigned mentoré·e·s. Until then, both wait on an "en attente" state.
  ...
  const title = isMentor ? "Mes mentoré·e·s" : assignedMentor ? "Mon binôme" : "Mon mentorat";
    ...
    <SafeAreaView className="flex-1 bg-bg" edges={["top"]}>
      <View className="px-[22px] pt-2 pb-3.5 flex-row justify-between items-center">
        ...
        <Pressable
          ...
          className="h-10 px-3 rounded-[14px] border border-border bg-card flex-row items-center gap-1.5 ml-3"
        >
      ...
      <ScrollView contentContainerClassName="pb-6">
        {isMentor ? (
          <>
            ...
            <View className="px-[22px]">
              <SectionEyebrow>Mes mentoré·e·s attribué·e·s</SectionEyebrow>
              {myMentees.length === 0 ? (
                <WaitingCard message="L'équipe PEEA t'attribuera un·e ou plusieurs mentoré·e·s prochainement. Tu les retrouveras ici." />
              ) : (
                <>
          ...
          <>
            ...
            <View className="px-[22px]">
              ...
              {assignedMentor ? (
                ...
                <WaitingCard message="L'équipe PEEA t'attribuera un mentor de ton domaine prochainement. Tu le retrouveras ici." />
              )}
            </View>
          </>
        )}
...
// Shown while the PEEA team has not yet attributed a mentor/mentee in the admin.
function WaitingCard({ message }: { message: string }) {
  return (
    <Card className="items-center p-6">
      <View className="w-[64px] h-[64px] rounded-[20px] mb-4 bg-primary-soft items-center justify-center">
        <Icon name="clock" size={28} color={colors.primaryDark} />
      </View>
      <Text className="text-[16px] font-poppins-bold text-fg text-center mb-1.5">En attente d'attribution</Text>
      <Text className="text-sm text-fg-muted leading-[21px] text-center">{message}</Text>
    </Card>
  );
}
